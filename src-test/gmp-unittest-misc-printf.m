...
#include "gmp-impl.h"
#include "tests.h"
#import "gmp-unittest-misc-printf.h"
...
@implementation gmp_unittest_misc_printf
...
-(void) check_vsprintf:(const char *)want :(const char *)fmt :(va_list) ap
{
  char  got[MAX_OUTPUT];
  int   got_len, want_len;
...
-(void) check_vfprintf:(const char *)want :(const char *)fmt :(va_list) ap
{
  char  got[MAX_OUTPUT];
...
-(void) check_vsnprintf:(const char *) want :(const char *)fmt :(va_list)ap
{
...
-(void) check_vasprintf:(const char *)want :(const char *)fmt :(va_list) ap
{
  char  *got;
  int   got_len, want_len;
...
-(void) check_obstack_vprintf:(const char *)want :(const char *)fmt :(va_list) ap
{
#if HAVE_OBSTACK_VPRINTF
  struct obstack  ob;
...
#endif
{
  ...
  /* simplest first */
  [self check_vsprintf:want:fmt:ap];
  [self check_vfprintf:want:fmt:ap];
  [self check_vsnprintf:want:fmt:ap];
  [self check_vasprintf:want:fmt:ap];
  [self check_obstack_vprintf:want:fmt:ap];
}
...
-(void) testCheckZ
{
  ...
  for (i = 0; i < numberof (data); i++)
  {
    ...
    {
      [self check_plain:data[i].want:data[i].fmt,mpz_get_si (z)];
    }
    
    [self check_one:data[i].want:data[i].fmt,z];
    ...
    for (zeros = 0; zeros <= 3; zeros++)
    {
      ...
      [self check_one:data[i].want:nfmt,PTR(z),nsize];
    }
...
-(void) testCheckQ
{
  ...
  for (i = 0; i < numberof (data); i++)
  {
    ...
    [self check_one:data[i].want:data[i].fmt,q];
  }
...
-(void) testCheckF
{
  ...
  for (i = 0; i < numberof (data); i++)
  {
    ...
    d = mpf_get_d (f);
    if (mpf_cmp_d (f, d) == 0)
      [self check_plain:data[i].want:data[i].fmt,d];
    
    [self check_one:data[i].want:data[i].fmt,f];
  }
...
-(void) testCheckLimb
{
  ...
  [self check_one:"0": "%Md", CNST_LIMB(0)];
  [self check_one:"1": "%Md", CNST_LIMB(1)];
  ...
  for (i = 1; i <= GMP_LIMB_BITS; i++)
  {
    ...
    [self check_one:s: "%Mu", limb];
    (*__gmp_free_func) (s, strlen (s) + 1);
    
    s = mpz_get_str (NULL, 16, z);
     [self check_one:s: "%Mx", limb];
    (*__gmp_free_func) (s, strlen (s) + 1);
...
-(void) testCheckN
{
  {
    int  n = -1;
    [self check_one:"blah": "%nblah", &n];
    ASSERT_ALWAYS (n == 0);
  }
  
  {
    int  n = -1;
    [self check_one:"hello ": "hello %n", &n];
    ASSERT_ALWAYS (n == 6);
  }
  
  {
    int  n = -1;
    [self check_one:"hello  world": "hello %n world", &n];
    ASSERT_ALWAYS (n == 6);
  }
...
[self check_one:"123456": fmt, 123, &x[0], 456];         \
\
...
\
  ...
  
  {
    ...
    [self check_one:"123456": "%d%Zn%d", 123, x[0], 456];
  ...
  
  {
    ...
    [self check_one:"123456": "%d%Qn%d", 123, x[0], 456];
  ...
  
  {
    ...
    mpf_set_ui (x[0], 987L);
    mpf_set_ui (x[1], 654L);
    [self check_one:"123456": "%d%Fn%d", 123, x[0], 456];
...
-(void) testCheckMisc
{
  ...
  mpz_set_ui (z, 12345L);
  [self check_one:"     12345": "%*Zd", 10, z];
  [self check_one:"0000012345": "%0*Zd", 10, z];
  [self check_one:"12345     ": "%*Zd", -10, z];
  [self check_one:"12345 and 678": "%Zd and %d", z, 678];
  [self check_one:"12345,1,12345,2,12345": "%Zd,%d,%Zd,%d,%Zd", z, 1, z, 2, z];
  
  /* from the glibc info docs */
  mpz_set_si (z, 0L);
  [self check_one:"|    0|0    |   +0|+0   |    0|00000|     |   00|0|":
             "|%5Zd|%-5Zd|%+5Zd|%+-5Zd|% 5Zd|%05Zd|%5.0Zd|%5.2Zd|%Zd|",
             /**/ z,    z,    z,     z,    z,    z,     z,     z,  z];
  mpz_set_si (z, 1L);
  [self check_one:"|    1|1    |   +1|+1   |    1|00001|    1|   01|1|":
             "|%5Zd|%-5Zd|%+5Zd|%+-5Zd|% 5Zd|%05Zd|%5.0Zd|%5.2Zd|%Zd|",
             /**/ z,    z,    z,     z,    z,    z,     z,     z,  z];
  mpz_set_si (z, -1L);
  [self check_one:"|   -1|-1   |   -1|-1   |   -1|-0001|   -1|  -01|-1|":
             "|%5Zd|%-5Zd|%+5Zd|%+-5Zd|% 5Zd|%05Zd|%5.0Zd|%5.2Zd|%Zd|",
             /**/ z,    z,    z,     z,    z,    z,     z,     z,  z];
  mpz_set_si (z, 100000L);
  [self check_one:"|100000|100000|+100000|+100000| 100000|100000|100000|100000|100000|":
             "|%5Zd|%-5Zd|%+5Zd|%+-5Zd|% 5Zd|%05Zd|%5.0Zd|%5.2Zd|%Zd|",
             /**/ z,    z,    z,     z,    z,    z,     z,     z,  z];
  mpz_set_si (z, 0L);
  [self check_one:"|    0|    0|    0|    0|    0|    0|  00000000|":
             "|%5Zo|%5Zx|%5ZX|%#5Zo|%#5Zx|%#5ZX|%#10.8Zx|",
             /**/ z,   z,   z,    z,    z,    z,       z];
  mpz_set_si (z, 1L);
  [self check_one:"|    1|    1|    1|   01|  0x1|  0X1|0x00000001|":
             "|%5Zo|%5Zx|%5ZX|%#5Zo|%#5Zx|%#5ZX|%#10.8Zx|",
             /**/ z,   z,   z,    z,    z,    z,       z];
  mpz_set_si (z, 100000L);
  [self check_one:"|303240|186a0|186A0|0303240|0x186a0|0X186A0|0x000186a0|":
             "|%5Zo|%5Zx|%5ZX|%#5Zo|%#5Zx|%#5ZX|%#10.8Zx|",
             /**/ z,   z,   z,    z,    z,    z,       z];
  ...
  mpz_set_ui (z, 789L);
  [self check_one:"456 789 blah": "%zd %Zd blah", (size_t) 456, z];
#endif
