...
#import "gmp-unittest-mpn-bdiv.h"
...
@implementation gmp_unittest_mpn_bdiv
...
#define RANDFUNC mpz_rrandomb
      ...
      if (nn > dn)
	    {
	      ...
	      [self check_one:qp: rp + nn - dn: rh: np: nn: dp: dn: "mpn_sbpi1_bdiv_qr"];
	    }
      ...
      if (nn > dn)
	    {
	      ...
	      ASSERT_ALWAYS (qp[-1] == qran0);  ASSERT_ALWAYS (qp[nn - dn + 1] == qran1);
	      ASSERT_ALWAYS (rp[-1] == rran0);
	      [self check_one:qp: NULL: 0: np: nn: dp: dn: "mpn_sbpi1_bdiv_q"];
	    }
    }
    ...
    if (dn >= 4 && nn - dn >= 2)
    {
      ...
      ASSERT_ALWAYS (qp[-1] == qran0);  ASSERT_ALWAYS (qp[nn - dn + 1] == qran1);
      ASSERT_ALWAYS (rp[-1] == rran0);
      [self check_one:qp: rp + nn - dn: rh: np: nn: dp: dn: "mpn_dcpi1_bdiv_qr"];
    }
    ...
    if (dn >= 4 && nn - dn >= 2)
    {
      ...
      ASSERT_ALWAYS (qp[-1] == qran0);  ASSERT_ALWAYS (qp[nn - dn + 1] == qran1);
      ASSERT_ALWAYS (rp[-1] == rran0);
      [self check_one:qp: NULL: 0: np: nn: dp: dn: "mpn_dcpi1_bdiv_q"];
    }
    ...
    [self check_one:qp: rp: rh: np: nn: dp: dn: "mpn_mu_bdiv_qr"];
    ...
    ASSERT_ALWAYS (ran == scratch[itch]);
    ASSERT_ALWAYS (qp[-1] == qran0);  ASSERT_ALWAYS (qp[nn - dn + 1] == qran1);
    [self check_one:qp: NULL: 0: np: nn: dp: dn: "mpn_mu_bdiv_q"];
  }
