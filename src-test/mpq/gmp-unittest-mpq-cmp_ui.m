...
#include "gmp-impl.h"
#include "tests.h"
#import "gmp-unittest-mpq-cmp_ui.h"
...
@implementation gmp_unittest_mpq_cmp_ui
/**
 *
 */
-(void) setUp
 ...
 *
 */
-(int) ref_mpq_cmp_ui:(mpq_t) a :(unsigned long int)bn :(unsigned long int) bd
{
  mpz_t ai, bi;
  int cc;
...
-(void) testCmpUi
{
  ...
  for (i = 0; i < reps; i++)
  {
    ...
    ccref = [self ref_mpq_cmp_ui:a :bn :bd];
    cc = mpq_cmp_ui (a, bn, bd);
    
    if (SGN (ccref) != SGN (cc))
      STFail(@"testCmpUi failed: SGN (ccref) != SGN (cc)");
  }
